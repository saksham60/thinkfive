...
class FixtureDashboardRepository implements DashboardRepository {
  ...
  Future<CustomerDashboardEntity> getCustomerDashboard() async {
    ...
    return CustomerDashboardEntity(
      accountSummary: const AccountSummary(balance: 14250.50, currency: 'USD', status: 'ACTIVE'),
      recentTransactions: [
        TransactionEntity(
          id: 't_1', merchant: 'Apple Store', amount: 1299.00, currency: 'USD',
          timestamp: DateTime.now().subtract(const Duration(hours: 2)), status: 'COMPLETED',
          hasFraudRisk: true,
        ),
        TransactionEntity(
          id: 't_2', merchant: 'Whole Foods', amount: 84.20, currency: 'USD',
          timestamp: DateTime.now().subtract(const Duration(hours: 24)), status: 'COMPLETED',
          hasFraudRisk: false,
        ),
      ],
      fraudAlerts: const [
